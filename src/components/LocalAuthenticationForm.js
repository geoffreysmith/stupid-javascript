...
export default React.createClass({
  ...
  render() {
    ...
    return ( < div className = "local-signin-form" >
      < form action = {
        this.signup
      } >
      < div className = "signup-options text-center form" >
      < div className = {
        this.formClassNames('username')
      } >
      < input type = "text"
      className = "form-control"
      placeholder = "Your Username"
      valueLink = {
        this.linkState('username')
      }
      /> {
        this.props.errors.username &&
          < span className = "label label-danger animate bounceIn" > {
            this.props.errors.username[0]
          } < /span>
      } < /div>

      < div className = {
        this.formClassNames('password')
      } >
      < input type = {
        passwordFieldType
      }
      ...
      < button className = "btn btn-lg btn-primary btn-signup"
      onClick = {
        this.signup
      } > {
